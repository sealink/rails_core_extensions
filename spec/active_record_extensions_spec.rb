...
describe "ActiveRecord::Base" do
  ...
  it "should update record if new_or_update! is passed hash with :id" do
    ...
    expect(Model).to receive(:find).and_return(@mock_model)
...
describe RailsCoreExtensions::ActionControllerSortable do
  ...
  it 'should sort' do
    expect(NormalController.new.methods).to_not include(:sort)
    expect(SortableController.new.methods).to include(:sort)
  end
end
