...
describe Concurrency do
  ...
  it "should allow specifying which methods should implement the concurrency check" do
    expect { ConcurrencyTest.send(:concurrency_safe, :instance_test_method) }.to_not raise_error
    expect { ConcurrencyTest.send(:concurrency_safe, :class_test_method) }.to_not raise_error
    expect { ConcurrencyTest.send(:concurrency_safe, :both_instance_and_class_test_method) }.to raise_error(Concurrency::AmbiguousMethodException)
    expect { ConcurrencyTest.send(:concurrency_safe, :both_instance_and_class_test_method, :type => :instance) }.to_not raise_error
    expect { ConcurrencyTest.send(:concurrency_safe, :both_instance_and_class_test_method, :type => :class) }.to_not raise_error
    expect { ConcurrencyTest.send(:concurrency_safe, :unknown_method) }.to raise_error(Concurrency::NoMethodException)
  end
  ...
  it "should allow identyfying the type of a method" do
    ...
    expect { ConcurrencyTest.send(:method_type, :both_instance_and_class_test_method) }.to raise_error(Concurrency::AmbiguousMethodException)
    expect { ConcurrencyTest.send(:method_type, :unknown_method) }.to raise_error(Concurrency::NoMethodException)
  end
  ...
  it "should allow checking the concurrency lock for specified class methods" do
    ...
    started = false
    expect(ConcurrencyTest.concurrency_safe_method_locked?(:class_test_method)).to eq false
    thread = Thread.new { ConcurrencyTest.send(:class_test_method); started = true }
    thread.join
    expect(ConcurrencyTest.concurrency_safe_method_locked?(:class_test_method)).to eq true until started
  end
  ...
  it "should allow checking the concurrency lock for specified instance methods" do
    ...
    started = false
    expect(instance.concurrency_safe_method_locked?(:instance_test_method)).to eq false
    thread = Thread.new { instance.send(:instance_test_method); started = true }
    thread.join
    expect(instance.concurrency_safe_method_locked?(:instance_test_method)).to eq true until started
  end
  ...
  it "should implement the concurrency check for specified class methods" do
    ...
    expect { threads.each(&:join) }.to raise_error(Concurrency::ConcurrentCallException)
  end
